#!/bin/bash
 ...
 if [[ "$DISK" >  "$DLIMIT" ]]; then
	 ...
	 mail -s 'Disk Space Alert' shellbashscripts@gmail.com << EOF
...
if [[ "$MEMORY" >  "$MLIMIT" ]];  then
        ...
        mail -s 'Memory Space Alert' shellbashscripts@gmail.com << EOF
...
if [[ "$CPU" >  "$CLIMIT" ]];  then
  	...
  	mail -s 'CPU Space Alert' shellbashscripts@gmail.com << EOF
